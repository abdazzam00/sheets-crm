...
export async function POST(req: Request) {
  try {
    ...
    const cacheKey = `ai:firm-niche:v1:${body.id}:${String(r.updated_at ?? '')}`;
    const cached = await aiCacheGet<{ firmNiche: string; reason: string }>(cacheKey);
    if (cached) {
      if (cached.firmNiche) {
        const updated = await updateRecord(body.id, { firmNiche: cached.firmNiche });
      ...
      messages: [
        {
          role: 'system',
          content:
            'Generate a concise firm niche description (5-12 words) based on company name/domain/notes. Return JSON {firmNiche, reason}. If unsure, keep firmNiche empty.',
        },
    ...
    const updated = out.firmNiche ? await updateRecord(body.id, { firmNiche: out.firmNiche }) : r;
