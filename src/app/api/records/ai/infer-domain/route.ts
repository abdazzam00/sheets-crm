...
export async function POST(req: Request) {
  try {
    ...
    const ctx = {
      ...
      currentDomain: String(r.domain ?? ''),
      notes: String(r.perplexity_research_notes ?? ''),
    };

    const cacheKey = `ai:infer-domain:v1:${body.id}:${String(r.updated_at ?? '')}`;
    const cached = await aiCacheGet<{ domain: string; reason: string }>(cacheKey);
    if (cached) {
      if (cached.domain) {
        const updated = await updateRecord(body.id, { domain: cached.domain });
      ...
      messages: [
        {
          role: 'system',
          content:
            'Infer the most likely company website domain (no protocol, no path). Return JSON {domain, reason}. If unsure, return empty domain. Never hallucinate wildly; be conservative.',
        },
    ...
    const updated = out.domain ? await updateRecord(body.id, { domain: out.domain }) : r;
